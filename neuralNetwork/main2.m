...
function main2(inputFileName, trainingSetQty, arquitecture, expectedOutputs, n, epsilon, learningEpsilon, epoques)
    ...
    for i=trainingSetQty + 1:patternsQty
        wishedoutput=patterns(i,1:end);
        %if (answerMultiLayer(weights,patterns(i,1:end-1),g,patterns(i,end)) <= learningEpsilon)
        %    learningPercentage = learningPercentage + 1;
        %end
        output=answerMultiLayer(weights,patterns(i,1:end-1),g,patterns(i,end));
        learningPercentage= learningPercentage+(wishedoutput-output)^2;
    end
    learningPercentage=learningPercentage/2;
    memorizationPercentage = memorizationPercentage / trainingSetQty
    learningPercentage = learningPercentage / epsilon
